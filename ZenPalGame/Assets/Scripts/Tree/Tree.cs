...
public class Tree : MonoBehaviour {
	...
	void MakeTree(Vector3 startPos, float angle, float scale, int depth)
	{
		...
		if(scale < TreeParms.minScale || depth >= TreeParms.maxDepth)
		...
		//make a left branch
		MakeTree( topPos,
		         angle - TreeParms.branchAngle + Random.Range(-TreeParms.angleRandom, TreeParms.angleRandom) ,
		         scale - (TreeParms.scaleChange + Random.Range(0, TreeParms.scaleRandom)),
		         depth+1);
